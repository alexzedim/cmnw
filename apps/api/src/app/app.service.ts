...
export class AppService {
  ...
  async getRaidLogsStats(input: RaidLogsStatsDto): Promise<IRaidLogsStats> {
    ...
    try {
      return await readThroughCache(
        this.redis,
        raidLogsCacheKey(input),
        { ttlSeconds: secondsUntilNextSnapshot() },
        async () => this.loadRaidLogsStats(input),
        (message) => this.logger.warn({ logTag, message }),
      );
    } catch (errorOrException) {
  ...
  private async loadRaidLogsStats(input: RaidLogsStatsDto): Promise<IRaidLogsStats> {
  ...
  async indexSearch(input: SearchQueryDto): Promise<ISearchResult> {
    ...
    try {
      ...
      return await readThroughCache(
        ...
        (message) => this.logger.warn({ logTag, searchQuery: input.searchQuery, message }),
      );
    } catch (errorOrException) {
      this.logger.error({
        logTag,
  ...
  private async performSearch(input: SearchQueryDto): Promise<ISearchResult> {
  ...
  async getLatestMetricSnapshot(snapshotQuery: AnalyticsMetricSnapshotDto): Promise<AnalyticsEntity | null> {
    ...
    try {
      return await readThroughCache(
        ...
        { ttlSeconds: secondsUntilNextSnapshot(), cacheNull: true },
        () => this.loadLatestMetricSnapshot(snapshotQuery),
        (message) => this.logger.warn({ logTag, category, metricType, realmId, message }),
      );
    } catch (errorOrException) {
      this.logger.error({
        logTag,
  ...
  private async loadLatestMetricSnapshot(snapshotQuery: AnalyticsMetricSnapshotDto): Promise<AnalyticsEntity | null> {
    const { category, metricType = AnalyticsMetricType.TOTAL, realmId } = snapshotQuery;
  ...
  async getMetricHistory(historyQuery: AnalyticsMetricHistoryDto): Promise<AnalyticsEntity[]> {
    ...
    try {
      return await readThroughCache(
        this.redis,
        historyCacheKey(historyQuery),
        { ttlSeconds: secondsUntilNextSnapshot() },
        () => this.loadMetricHistory(historyQuery),
        (message) => this.logger.warn({ logTag, category, metricType, realmId, fromDate, toDate, message }),
      );
    } catch (errorOrException) {
      this.logger.error({
        logTag,
  ...
  private async loadMetricHistory(historyQuery: AnalyticsMetricHistoryDto): Promise<AnalyticsEntity[]> {
    const { category, metricType = AnalyticsMetricType.TOTAL, realmId, fromDate, toDate } = historyQuery;
